var Inbox = {
	...
	select_none : function() {
		$('.message-item input[type=checkbox]').removeAttr('checked').closest(
	...
	select_unread : function() {
		$('.message-item:not(.message-unread) input[type=checkbox]')
				.removeAttr('checked').closest('.message-item').removeClass(
						'selected');
...
jQuery(function($) {
			...
			.on(
					...
					function() {
						...
						$('.message-inline-open').removeClass(
								'message-inline-open').find('.message-content')
								.remove();
						var message_list = $(this).closest('.message-list');
						// some waiting
						setTimeout(function() {
							// hide everything that is after .message-list
							// (which is either .message-content or
							// .message-form)
							message_list.next().addClass('hide');
							$('.message-container').find(
									'.message-loading-overlay').remove();
							// close and remove the inline opened message if
							// any!
							// hide all navbars
							...
							// move .message-content next to .message-list and
							// hide .message-list
							message_list.addClass('hide').after(
									$('.message-content')).next().removeClass(
									'hide');
			...
			.on(
					...
					function() {
						...
						if (message.hasClass('message-inline-open')) {
							message.removeClass('message-inline-open').find(
						...
						setTimeout(function() {
							...
							var content = message.find('.message-content:last')
									.html($('#id-message-content').html());
		...
		$('.message-form .wysiwyg-editor').ace_wysiwyg(
				{
					toolbar : [ 'bold', 'italic', 'strikethrough', 'underline',
							...
							'justifyright', null, 'createLink', 'unlink', null,
							'undo', 'redo' ]
		...
		$('#id-add-attachment')
				...
				<a href="#" data-action="delete" class="middle">\
					<i class="icon-trash red bigger-130 middle"></i>\
									...
									.find('a[data-action=delete]').on(
											'click',
											function(e) {
												// the button that removes the
												// newly inserted file input
												e.preventDefault();
	...
	/**
	 * var tag_input = $('#form-field-recipient'); if(! (
	 * /msie\s*(8|7|6)/.test(navigator.userAgent.toLowerCase())) )
	 * tag_input.tag({placeholder:tag_input.attr('placeholder')});
	 * 
	 * 
	 * //and add form reset functionality $('.message-form
	 * button[type=reset]').on('click', function(){ $('.message-form
	 * .message-body').empty();
	 * 
	 ...
	 * $(val.split(',')).each(function(k,v){ tag_input.before('<span
	 * class="tag">'+v+'<button class="close" type="button">&times;</button></span>');
	 * }); });
	 */
