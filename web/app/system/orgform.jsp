...
<form id="orgInfo_form" action="org/saveOrUpdate.htm" method="post" class="">
	...
	<input name="type" class="hide" value="3"/>
	<div class="profile-info-row">
		<div class="profile-info-name required">班级名称 </div>
		<div class="profile-info-value">
				<input name="name" class="editable" value="${org.name}"/>
		</div>
	</div>
	<div class="profile-info-row">
		<div class="profile-info-name required">班级编号 </div>
		<div class="profile-info-value">
				<input name="code" class="editable" value="${org.code}"/>
		</div>
	</div>
	<div class="profile-info-row">
		<div class="profile-info-name required">结业年 </div>
		<div class="profile-info-value">
				<input class="Wdate" name="date" value="${org.date}" type="text" onClick="WdatePicker({minDate:'%y-%M-%d'})">
		</div>
	</div>
	<div class="profile-info-row">
		<div class="profile-info-name required">属于年级</div>
		<div class="profile-info-value" id="orgParentId">
			<input name="parent.id" class="editable hidden" value="${orgParent.id}"/>${orgParent.name}
		</div>
	</div>
	<div class="profile-info-row">
		<div class="profile-info-name required">班主任</div>
		<div class="profile-info-value">
				<select name="mLeader">
					<c:forEach items="${leaders}" var="leader">
						<option value=",${leader[0]},">${leader[1]}</option>
					</c:forEach>
				</select>
		</div>
	</div>
