	...
	$.fn.fileUploader = function(config){
		...
		*/
		function afterEachUpload(formId, data, status, errorThrown) {
			...
			if (data) {
				...
				attachId=$(data).find('#attachId').text();
			}
			formId = pxWidget + ' #' + formId;
			$uploadData = $(formId + '_text');
			if (status == 'success'){
				$uploadData.removeClass('uploading').addClass('success');
				$uploadData.children('.status').html("状态:<font color='blue'>"+$(data).find('#message').text()+"</font>");
			} else if (status == 'error'){
				$uploadData.removeClass('uploading').addClass('error');
				$uploadData.children('.status').html("状态:<font color='red'>"+$(data).find('#message').text() +"</font>");
				...
				$uploadData.children('.status').html( '取消上传' );
			}
			
			$uploadData.find('.cancel').removeClass('cancel').addClass('delete').attr('title', '删除').attr("attachId",attachId);
			
			//hide progress bar
			$uploadData.find('.progress').hide();
			
			//trigger after each upload
			config.afterEachUpload(data,status,attachId,$uploadData);
		...
		*/
		$('.upload', pxUploadForm).live('click', function(){
			//console.log("开始上传"+isHtml5+"-----true google,false:IE7,8");
